...
"""
TODO the translate() etc.. functions referance is missing
"""
def levenshtein_alignment(model, tokenizer, evidence, translated_paragraph, target_language, original_position_ratio, lambda_ratio=0.8):
    # translate evidence's tokens
    translated_evidence = translate(model, tokenizer, [evidence], target_language)[0]
    translated_tokens = translated_evidence.split(' ')
    logging.info("CS TOKENS: {}".format(translated_tokens))
    ...
    logging.info("EN: {}".format(evidence))
    logging.info("CS: {} {}".format(translated_evidence, evidence_start))
    logging.info("PARAGRAPH: {}".format(translated_paragraph))
    logging.info("SCORE: {}".format(candidates[best_candidate_index]["score"]))
    logging.info("------------------------------------------------------------------------------")
    return translated_evidence, evidence_start, candidates[best_candidate_index]["score"]
