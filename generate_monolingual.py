...
directory_path = "jsons/monolingual/"
...
def print_md_tables(merged_data, print_final_json = False):
    new_data = {}
...
merged_data_inter = defaultdict(dict)
merged_data_full = defaultdict(dict)
...
for filename in os.listdir(directory_path):
    if filename.endswith(".json"):
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            data = json.load(file)
            if "ClinicalBERT" in data:
                data["xClinicalEN"] = data.pop("ClinicalBERT")
            if "BioBERT" in data:
                data["xBioBERT"] = data.pop("BioBERT")
            if "BERTbase" in data:
                data["xBERTbase"] = data.pop("BERTbase")
            if "#FULL" in language:
                merge_dicts(merged_data_full, data)
            else:
                merge_dicts(merged_data_inter, data)


# Convert defaultdict to a regular dict for pretty-printing
merged_data_inter = dict(merged_data_inter)
merged_data_full = dict(merged_data_full)


print("## INTERSECTION TESTSET")
print_md_tables(merged_data_inter)
print("## FULL TESTSET")
print_md_tables(merged_data_full)
